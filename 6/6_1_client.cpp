...
int main(int argc, char *argv[]) {
    ...
    int ret = recv(sockfd, recvbuf, BUFFER_SIZE, 0);
    for (int i = 0; i < ret; i++) {
        printf("%c", recvbuf[i]);
    }
    std::cout << std::endl;
    ...
    //         // ret = splice(0, NULL, pipefd[1], NULL, 32765, SPLICE_F_MORE | SPLICE_F_MOVE);
    //         // ret = splice(pipefd[0], NULL, sockfd, NULL, 32765, SPLICE_F_MORE | SPLICE_F_MOVE);
